...
use autoqueues::{SimpleQueue, QueueTrait};
...
async fn test_server_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Start server
    let server_handle = queue.clone().start_server()?;
    println!("   ✅ Server started successfully");

    // Test operations while server runs
    let mut queue_for_publish = queue.clone();
    for i in 1..=5 {
        queue_for_publish.publish(i * 10)?;
...
async fn test_concurrent_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..=5 {
        ...
        let handle = tokio::spawn(async move {
            for j in 1..=20 {
                let _ = queue_clone.clone().publish(i * 100 + j);
    ...
    println!("   Concurrent publishers: {}", handle_count);
