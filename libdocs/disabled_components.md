...
This file tracks components that have been temporarily disabled or not yet fully implemented.
...
- `src/cluster/node.rs` - RaftNode wrapper with ZMQ transport
...
1. **Full OpenRaft Integration**
   ...
   - RaftNetworkFactory implementation for ZMQ
...
| Port | Purpose |
|------|---------|
| 6967 (data port) | Queue data, pub/sub topics |
| 6968 (coordination port) | Raft/cluster coordination (ZMQ) |

## Next Steps

1. Complete OpenRaft type configuration with `declare_raft_types!`
2. Implement `RaftNetworkFactory` for ZMQ transport
