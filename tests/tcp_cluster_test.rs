...
async fn test_real_lease_release() {
    ...
    // Acquire lease
    leader
        .acquire_lease("resource:2", 60)
        .await
        .expect("Acquire failed");

    // Release lease
    leader
        .release_lease("resource:2")
        .await
...
async fn test_real_5_node_cluster() {
    ...
    let leader = cluster.get_node(1).unwrap();
    leader
        .claim_topic("large.cluster.topic")
        .await
        .expect("Claim failed");
...
async fn test_real_topic_transfer() {
    ...
    // Transfer to node 2
    leader
        .transfer_topic("transfer.topic", 2)
        .await
