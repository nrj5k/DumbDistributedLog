...
async fn test_function_source_pause_resume() {
    ...
    assert!(
        count_before > 0,
        "Should have generated some data initially"
    );
    ...
    assert_eq!(
        count_during_pause, count_before,
    ...
    assert!(
        count_after > count_during_pause,
        "Should increment after resume"
    );
}
...
async fn test_function_source_remove() {
    ...
    assert!(
        count_after_removal - count_when_removed <= 2,
        "Should not increment much after removal"
    );
}
...
async fn test_function_source_integration() {
    ...
    assert!(
        after_pause_count - paused_count <= 2,
        "Should not increment much while paused"
    );
