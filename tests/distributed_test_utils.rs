...
impl TestCluster {
    ...
    pub async fn setup_tcp_with_options(
        ...
        for (idx, (node_id, config)) in configs.iter().enumerate() {
            eprintln!(
                "setup_tcp: Creating node {} at {}:{}",
                node_id, config.host, config.coordination_port
            );

            let mut network_config =
                ddl::network::TcpNetworkConfig::new(*node_id, config.coordination_port);

            // Add all other peers
            for (peer_id, peer_config) in &configs {
                if peer_id != node_id {
                    network_config.add_peer(
                        *peer_id,
                        format!("{}:{}", peer_config.host, peer_config.coordination_port),
                    );
                }
            }
        ...
        let is_leader = tokio::time::timeout(tokio::time::Duration::from_secs(10), async {
            loop {
        ...
        .await
        .map_err(|_| "Leader election timeout")?;
