...
impl ChaosState {
    fn check_panic(&self) {
        if self.should_panic.load(Ordering::Acquire) {
            ...
            panic!("Chaos: Intentional panic at location {}", self.panic_locations.load(Ordering::Acquire));
        }
    }
}
...
fn create_chaos_lock() -> (RecoverableLock<ChaosState>, Arc<AtomicBool>, Arc<AtomicUsize>) {
...
fn test_lock_poison_cascade() {
    ...
    assert!(result_c.is_ok() || matches!(result_c, Err(LockError::Poisoned { .. })), 
...
fn test_concurrent_poison_scenarios() {
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            let mut guard = lock_clone.write_recover(&format!("concurrent_thread_{}", i)).unwrap();
...
fn test_ownership_state_poison_recovery() {
    ...
    assert_eq!(ownership_state.read_recover().unwrap().get_owner("metrics.test"), Some(1));
...
fn test_poison_count_limit_enforcement() {
    ...
    assert!(lock.poison_count() >= limit, "Poison count should reach limit");
}
