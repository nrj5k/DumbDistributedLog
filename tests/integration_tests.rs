...
fn test_cluster_partition_recovery() {
    ...
    assert_eq!(state.topic_count(), 2, "Should have both topics after merge");
}
...
fn test_snapshot_transfer_size_limit() {
    ...
    if estimate_size > MAX_MESSAGE_SIZE {
        ...
        eprintln!("Snapshot would exceed MAX_MESSAGE_SIZE ({})", MAX_MESSAGE_SIZE);
    } else {
...
fn test_cluster_reintegration() {
    ...
    // Verify no data loss
    assert!(merged.topic_count() >= 1, "Should have topics after reintegration");
}
