...
use distributed_test_utils::{
    get_node_spec_from_env, get_node_spec_or_default, parse_node_spec, validate_node_spec,
    NodeSpecBuilder, TestCluster, DEFAULT_COMMUNICATION_PORT,
};
...
async fn test_with_named_hosts_range() {
    ...
    assert!(cluster
        ...
        .unwrap()
        .host
        .contains("ares-comp-13"));
    assert!(cluster
        .configs
        .get(&4)
        .unwrap()
        .host
        .contains("ares-comp-16"));
...
async fn test_with_explicit_list() {
    ...
    let configs = NodeSpecBuilder::new(spec).build().unwrap();
...
async fn test_with_ip_range() {
    ...
    let cluster =
        TestCluster::setup_with_options("10.0.0.[1-4]:9090", DEFAULT_COMMUNICATION_PORT, 1)
            .await
            .expect("Failed to create cluster");
...
fn test_leading_zeros_preserved() {
    ...
    let configs = NodeSpecBuilder::new("node[001-003]").build().unwrap();
