...
impl Default for LeaderConfig {
    fn default() -> Self {
        Self {
            ...
            aggregation_interval_ms: constants::time::AGGREGATION_INTERVAL_MS,
            peer_nodes: Vec::new(),
        }
    }
}
...
impl Leader {
    pub fn new(
        ...
        let _ = subscribe_socket.set_rcvtimeo(constants::network::ZMQ_RCVTIMEO_MS);
        ...
        for peer in &config.peer_nodes {
            ...
            let max_retries = constants::time::MAX_RETRIES;
                    ...
                    Err(e) => {
                        ...
                        if retry_count >= max_retries {
                            eprintln!(
                                ...
                                peer_addr, max_retries, e
                            );
                            break;
                        }
                        std::thread::sleep(Duration::from_millis(
                            constants::time::RETRY_BASE_DELAY_MS * retry_count as u64,
                        ));
                    }
                }
            }
        ...
        let publish_addr = format!(
            "tcp://{}:{}",
            config.bind_addr,
            config.data_port + constants::system::BASE_PORT_OFFSET
        );
    ...
    pub async fn run(&mut self) {
        let mut aggregation_interval =
            interval(Duration::from_millis(self.config.aggregation_interval_ms));
    ...
    fn run_query_handler(socket: Socket, state: Arc<RwLock<LeaderState>>) {
        loop {
            match socket.recv_string(0) {
                Ok(Ok(request)) => {
                    let response = if request.starts_with("get_metric:") {
                        ...
                        match state.read() {
                            Ok(guard) => match guard.aggregated_values.get(metric) {
    ...
    fn run_subscription_handler(peers: Vec<String>, state: Arc<RwLock<LeaderState>>) {
        ...
        let _ = socket.set_linger(0);
        let _ = socket.set_rcvtimeo(constants::network::ZMQ_SHORT_RCVTIMEO_MS);
            ...
            while retry_count < max_retries {
                match socket.connect(&peer_addr) {
                    ...
                    Err(e) => {
                        ...
                        if retry_count >= max_retries {
                            eprintln!(
                                "Failed to connect subscriber to peer {} after {} retries: {}",
                                peer_addr, max_retries, e
                            );
                            break;
                        }
        ...
        loop {
            match socket.recv_string(0) {
                            ...
                            if let Ok(value) = parts[2].parse::<f64>() {
                                match state.write() {
                                    Ok(mut guard) => {
                                        ...
                                        if entry.len()
                                            > constants::memory::SMALL_HISTORY_BUFFER_SIZE
                                        {
                                            entry.drain(
                                                0..(entry.len()
                                                    - constants::memory::SMALL_HISTORY_BUFFER_SIZE),
                ...
                Ok(Err(_)) => {}
                Err(zmq::Error::EAGAIN) => std::thread::sleep(Duration::from_millis(
                    constants::time::VERY_SHORT_SLEEP_INTERVAL_MS,
                )),
    ...
    fn aggregate_metric(&self, metric: &DerivedMetric, state: &mut LeaderState) {
        ...
        state
            .aggregated_values
            .insert(metric.name.clone(), aggregated);
    }
    ...
    pub fn update_local_value(&mut self, value: LocalMetricValue) {
        match self.state.write() {
            Ok(mut state) => {
                ...
                if entry.len() > constants::memory::SMALL_HISTORY_BUFFER_SIZE {
                    entry.drain(0..(entry.len() - constants::memory::SMALL_HISTORY_BUFFER_SIZE));
                }
            }
            Err(e) => {
