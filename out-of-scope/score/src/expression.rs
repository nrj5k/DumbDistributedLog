...
pub trait Expression<T>: Send + Sync {
    fn convert_syntax(&self, expr: &str) -> String {
        expr.replace("atomic.", "atomic_")
...
#[derive(Debug, Clone)]
pub struct SimpleExpression<T> where T: From<f64> + Sync {
...
{
    /// Convert expression syntax (atomic. -> atomic_, cluster. -> cluster_)
    fn convert_syntax(expr: &str) -> String {
        expr.replace("atomic.", "atomic_")
            .replace("cluster.", "cluster_")
    }
    ...
    pub fn new(expression: &str) -> Result<Self, ExpressionError> {
        ...
        let converted = Self::convert_syntax(expr);
    ...
    fn validate_expression(&self) -> Result<(), ExpressionError> {
        ...
        if expr.len() > constants::expression::MAX_EXPRESSION_LENGTH {
            return Err(ExpressionError::ParseError(format!(
                "Expression too long (max {} characters)",
                constants::expression::MAX_EXPRESSION_LENGTH
            )));
        }
    ...
    fn validate_supported_functions(&self, expr: &str) -> Result<(), ExpressionError> {
        ...
        let function_pattern = regex::Regex::new(r"(\w+)\s*\(")
            .map_err(|e| ExpressionError::ParseError(format!("Invalid regex pattern: {}", e)))?;
    ...
    fn validate_variable_syntax(&self, expr: &str) -> Result<(), ExpressionError> {
        let atomic_var_pattern = regex::Regex::new(r"atomic\.([a-zA-Z_][a-zA-Z0-9_]*)")
            .map_err(|e| ExpressionError::ParseError(format!("Invalid regex pattern: {}", e)))?;
        ...
        let cluster_var_pattern = regex::Regex::new(r"cluster\.([a-zA-Z_][a-zA-Z0-9_]*)")
            .map_err(|e| ExpressionError::ParseError(format!("Invalid regex pattern: {}", e)))?;
    ...
    T: Clone + Send + Sync + 'static + From<f64>,
{
    fn evaluate(
        ...
        match eval_with_context(&self.converted, &context) {
            Ok(result) => result
                .as_float()
                .map(|f| T::from(f))
